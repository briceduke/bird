...
const HomePage: NextPage = () => {
	...
	const { data: timelineData, error, refetch } = useGetHomeTimelineQuery({ limit: 20, skip: 0 });
			...
			<div className="p-4">
				<div className='w-full h-72'>
					<Formik
						...
						onSubmit={(values: CreateChirpPayload,
					...
					<Form className="h-full flex justify-evenly flex-col">
						...
						<button
							...
							className={`btn btn-primary ${isSubmitting ? "loading" : ""} w-20`}
						>
							Chirp
						</button>
						...
						{isError && (
							<div className="text-error">
								<p>{(createError as any).data && (createError as any).data.message ? (createError as any).data.message : JSON.stringify(createError)}</p>
							</div>
						)}
					</Form>
				)}
				...
				<div>
					<button className='btn btn-primary' onClick={handleGenInvite}>Create Invite Code</button>
					{JSON.stringify(inviteCode)}
				</div>
				<div>
					{error && <p className='text-error'>{JSON.stringify(error)}</p>}
					{timelineData && timelineData.map((chirp, idx) => {
						return (
							<div key={idx} className='flex'>
								...
								<div className='flex flex-col items-baseline justify-evenly w-5/6'>
									{/* Name and username */}
									<div className='flex'>
										<p className='text-white font-bold'>{chirp.userUsername}</p>
									</div>
									...
									<div className='w-full flex items-center justify-evenly'>
										<Link href={`/chirp/${chirp._id}`}>
											<button className={`btn btn-square btn-${chirp.likedUserIds.includes(data._id) ? 'primary' : 'ghost'}`}>{chirp.likeCount} Likes</button>
										</Link>
									</div>
								</div>
							</div>
