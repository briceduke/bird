...
export const FollowersPage = ({ username }: ProfileProps) => {
	const { data } = useGetUserQuery({ username });
	const [getUsersById, { data: followersData }] = useLazyGetUsersByIdQuery();

	useEffect(() => {
		const getUsers = async () => {
			await getUsersById({ _id: data.followersId });
		};
	...
	return (
		<div>
			{data && (
				<Layout
					protectedRoute
					title={`People following ${data.displayName} (@${data.username})`}
				>
					<div className="p-10">
